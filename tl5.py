...
def process_images_in_folder(folder_path, output_file, input_size=448, max_num=6, limit=10):
    # 确保文件夹路径存在
    ...
    image_count = 0
    image_files = []
    for f in sorted(os.listdir(folder_path), key=extract_image_number):
    ...
    with open(output_file, 'w') as f:
        for image_file in image_files:
            ...
            # 准备问题和生成配置
            questions = ['<image>\n描述图片上的人的穿着','图中人物性别为男性则输出0，女性则输出1']
            generation_config = {
                'num_beams': 1,
                'max_new_tokens': 1024,
                'do_sample': False,
            ...
            f.write(f'{image_number}: {responses}\n')
            print(f'Processed {image_file}')

# 调用函数处理指定文件夹中的图片
process_images_in_folder("/home/develop/InternVL/image_test_internvl/", "output5.txt")
